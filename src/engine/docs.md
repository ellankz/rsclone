...
Главный класс создающий движок и ргулирующий все методы.
...
```javascript
{
  ...
  canvasOffset: Vector,  // отступ контейнера от ганиц окна
...
1. Методы прямой отрисовки (не рекомендуются). Не сохраняються в памяти, полность статичны, нет возможности вернуть после очистки canvas.
...
В конструкторе сразу можно можно создавать узлы, или использовать уже созданные, так как после вызова конструктора остануться только методы. Все коллбеки как и конструктор не обязательные.
...
Можно создать камеру отслеживающую несколько слоев (например, слои являються частью одного экрана или их нужно перемещать одновременно). В этом случае прошлая камера перезатирается. При создании координаты камеры равны `{x: 0; y: 0}`
